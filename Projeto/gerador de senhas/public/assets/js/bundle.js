...
var tamanhoSenha = document.querySelector(".tamanho-senha");
...
function gera() {
  return (0,_geradores__WEBPACK_IMPORTED_MODULE_0__["default"])(tamanhoSenha.value, chkMaiusculas.checked, chkMinusculas.checked, chkNumeros.checked, chkSimbolos.checked);
}
...
function geraSenha(qtd, maiusculas, minusculas, numeros, simbolos) {
  var senhaArray = [];
  qtd = Number(qtd);
  if (qtd === 0) return 'Tamanho inválido';
  for (var i = 0; i < qtd; i++) {
  ...
  return senhaArray.join('').slice(0, qtd) || 'Nada selecionado!';
}
